...
def parse_repo_info(env_var_name):
    repo_info = os.getenv(env_var_name)
    if not repo_info:
        raise ValueError(f"{env_var_name} environment variable is not set.")

    parts = repo_info.split(',')
    if len(parts) < 2:
        raise ValueError(f"{env_var_name} environment variable must contain at least username and repository name.")

    username = parts[0].strip()
    repository = parts[1].strip()
    branch = parts[2].strip() if len(parts) > 2 else 'main'
    logger.debug(f"Repository: {username}/{repository} branch: {branch}, being used for {env_var_name}")
