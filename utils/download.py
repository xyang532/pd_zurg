...
class Downloader:
    ...
    def handle_rate_limits(self, response):
        if response.status_code in [403, 429]:
            if 'Retry-After' in response.headers:
                ...
                self.logger.warning(f"Rate limit exceeded. Retrying after {sleep_time} seconds.")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not self.handle_rate_limits(response):
                    break
                self.logger.info(f"Retry attempt {attempt + 1} after rate limit handling.")
    ...
    def download_and_extract(self, url, target_dir, zip_folder_name=None, headers=None, exclude_dirs=None):
        try:
            ...
            if response and response.status_code == 200:
                self.logger.debug(f"{zip_folder_name} download successful. Content size: {len(response.content)} bytes")
                try:
